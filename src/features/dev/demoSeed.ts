 ...
 *
 * Idempotent: a note is matched to its session by timestamp and rewritten in
 * place rather than duplicated, and `recordReview` ignores a duplicate tick, so
 * an interrupted run resumes cleanly and a content change lands on reseed.
 */
export async function seedDemoData(
    ...
    for (const planned of plan) {
        ...
        const sealed = await encryptNoteText(planned.text);

        if (existing) {
            // Rewrite rather than skip. When the seeded copy changes, the row
            // for that session already exists and skipping would leave the
            // device showing the previous version forever. Updating in place
            // keeps the note id, and so keeps the review rows pointing at it.
            await db.runAsync(
