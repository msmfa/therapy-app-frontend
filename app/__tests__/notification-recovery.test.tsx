...
jest.mock('../../src/services/notifications/pushRegistration', () => ({ ensurePushRegistration: () => mockRegister() }));
...
import ReminderSettingsScreen from '../../app/reminder-settings';
...
describe('reminder settings', () => {
	...
	it('requests notification permission after onboarding was skipped', async () => {
        ...
        mockRegister.mockResolvedValue({ status: 'registered', token: 'fake' });
        const open = jest.spyOn(Linking, 'openSettings').mockResolvedValue();
        const { getByText } = render(<ReminderSettingsScreen />);
        await waitFor(() => expect(getByText('Notification permissions: Off')).not.toBeNull());
        fireEvent.press(getByText('Notification permissions: Off'));
