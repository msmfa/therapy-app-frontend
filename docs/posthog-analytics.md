...
Measure whether people save something useful, return to review it, and keep doing so. In the public app, analytics is optional and off until a person chooses **Share app usage** on Welcome or in Account settings. The current TestFlight cohort has already consented to beta analytics: its dedicated build enables that preference locally and hides this control and its explanatory text. This implementation collects a small explicit event vocabulary, without screen/tap autocapture or session replay.

## Deployment and configuration

1. Deploy the companion backend analytics changes first. `GET /api/users/me` returns optional `analyticsConsent`; `PATCH /api/users/me` accepts a strict boolean. Existing accounts have no implicit consent. The backend also supplies verified subscription events and durable analytics deletion. See the backend analytics guide for its worker and environment configuration.
...
4. The `analytics-qa` EAS profile is a Release simulator build against the staging API. The `testflight` profile is a physical-device Release build with store distribution against the production API and production EAS environment. Both set `EXPO_PUBLIC_ANALYTICS_ENVIRONMENT=qa`; beta/test events carry `environment=qa`. Only `testflight` also sets `EXPO_PUBLIC_ANALYTICS_TESTFLIGHT_PRECONSENT=1`. Its prior-consent policy requires both flags and uses `.qa.testflight` consent, queue and engagement storage, isolated from production and ordinary `.qa` consent testing. TestFlight never reads or updates the backend analytics-consent preference; accounts and other API records still use production. The `production` profile explicitly sets `environment=production` and pre-consent `0`; `analytics-qa` inherits pre-consent `0` and retains explicit opt-in. The current PostHog plan allows one project, so both beta profiles share project 260159 while every production insight/cohort explicitly excludes QA events. Development builds, demo seeding, subscription fixtures, and builds with `EXPO_PUBLIC_ANALYTICS_INTERNAL_USER=true` cannot collect.
5. Follow the [TestFlight release procedure](testflight-release.md) for build/upload commands, scoped Sandbox access and tester priorities. Verify opt-in, logout/login, account switching, offline retry and account deletion on a device. Inspect raw events filtered to `environment=qa` to confirm only the dictionary below arrives. Native StoreKit behaviour still needs device/sandbox verification; sandbox billing is excluded from production lifecycle analytics. Before public release, rebuild with the `production` profile and verify production analytics configuration; do not promote the QA-tagged beta binary directly.
...
Consent is saved per account on the device and synchronized to the backend on authentication and foreground entry. A never-chosen anonymous preference may transfer to the first account, but one signed-in account's choice is never inherited by another. A new device reads an existing server choice. Offline changes take effect locally immediately and retry server synchronization; other devices/backend delivery use their last synchronized preference until that request arrives. The UI explains when this sync is pending.

The pre-consented TestFlight build is the exception to account synchronization: after authentication resolves, its local beta preference defaults to true for anonymous onboarding and each canonical account. It does not fabricate a public-app consent record or copy this preference into a later production build. An existing explicit false in its own beta namespace still wins, and storage failure leaves capture disabled. Normal account-switch invalidation, stable identities, queue cleanup and deletion remain in effect. Use `analytics-qa` to exercise the public opt-in/opt-out interface. Restrict the pre-consented TestFlight build to testers whose prior agreement has been obtained; a wider beta cohort needs its own consent step or the flag set to `0`.

Opt-out stops future local collection and clears queued events and local engagement metadata. It does not delete previously uploaded events. Account deletion clears device analytics state and invokes the companion backend's durable PostHog erasure flow. Server-only deletion credentials and a running worker are required for remote erasure; do not claim remote deletion is complete just because the local app reset.
...
Create a dashboard named **Plastic Brains — meaningful use**, use unique people where indicated, filter to production, and exclude designated internal/test cohorts. All product metrics represent the opted-in population, not all customers. Billing records remain authoritative for revenue and subscriber totals.
...
- Client SDK buffering is bounded to 100 events and preserves occurrence timestamps and UUIDs on ordinary same-account offline retries. Logout, account switch, opt-out and deletion deliberately discard unsent events. Device loss, process/storage failure and queue eviction can lose events; this is product analytics, not an audit ledger.
- There is no historical event backfill when a person opts in. Billing delivery must also pass the current server consent and production-account checks.
- An automatic checkout after authentication waits up to one second for that account's local consent and SDK initialization before the checkout starts. Unknown/declined consent, initialization failure, or timeout proceeds with billing and permanently omits that attempt's client analytics; no pre-consent activity is buffered. An account change during the wait cancels the old attempt. Use verified backend billing for subscription totals, since slow initialization can still undercount the client funnel.
