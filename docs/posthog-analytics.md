...
4. Use a separate PostHog project and release build for QA. Development builds, demo seeding, subscription fixtures, and builds with `EXPO_PUBLIC_ANALYTICS_INTERNAL_USER=true` cannot collect. Keep staff/test builds excluded from production data.
...
6. In PostHog, configure the intended retention period, project access and the dashboard/cohorts below. No PostHog project, credentials, retention setting or hosted dashboard is created by this source change.
...
- An automatic checkout immediately after authentication can begin before the destination account's consent/SDK activation finishes. That client operation is omitted instead of buffering activity before permission is known. Use verified backend billing for subscription totals; the client checkout funnel can undercount this handoff.
