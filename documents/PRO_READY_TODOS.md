...
- [x] Remove the seeded test email and password from the login form defaults so production builds never leak test credentials. 【F:app/(auth)/login.tsx†L27-L46】
...
- [x] Add centralized error and crash reporting (e.g. Sentry) instead of only logging to the console when authentication, notification, or storage calls fail. 【F:src/context/auth/AuthContext.tsx†L97-L205】【F:app/\_layout.tsx†L105-L110】
...
- [x] Add a Terms of Service screen (and settings link) to cover required legal disclosures. 【F:app/(tabs)/settings.tsx†L64-L96】
